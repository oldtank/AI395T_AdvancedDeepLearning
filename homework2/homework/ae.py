...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchDecoder(torch.nn.Module):
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            ...
            self.deconv1 = torch.nn.ConvTranspose2d(bottleneck, bottleneck * 2, kernel_size=4, stride=2, padding=1)
            self.deconv2 = torch.nn.ConvTranspose2d(bottleneck * 2, latent_dim, kernel_size=4, stride=2, padding=1)
            self.deconv3 = torch.nn.ConvTranspose2d(latent_dim, 3, kernel_size=patch_size+1, stride=patch_size)
